...
public abstract partial class SharedProjectileSystem : EntitySystem
{
    ...
    // God Bless DeepSeek goida code
    private void OnStartCollide(EntityUid uid, ProjectileComponent component, ref StartCollideEvent args)
    {
        // Skip if collision invalid (shared checks)
        if (args.OurFixtureId != ProjectileFixture || !args.OtherFixture.Hard
            || component.DamagedEntity || component is { Weapon: null, OnlyCollideWhenShot: true })
            return;

        // Mark as damaged IMMEDIATELY to prevent duplicate processing
        component.DamagedEntity = true;
        Dirty(uid, component);

        RaiseLocalEvent(new ProjectileCollideEvent(uid, component, args.OtherEntity, args.OurBody));

        // Shared cleanup (delete projectile)
